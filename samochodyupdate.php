<?php
    ...
    if ( !empty($_POST)) {
        ...
        if (empty($MARKA)) {
            $MARKAError = 'Podaj marke';
            $valid = false;
        }
        $marki = array('SKODA','BMW','OPEL', 'TOYOTA', 'KIA', 'FIAT', 'FORD');
        
        if(!in_array(strtoupper($MARKA), $marki)){
        ...
        if ($valid) {
            ...
            $sql = "UPDATE SAMOCHODY set REJESTRACJA = ?, CZY_USZKODZONY = ?, MARKA = ? WHERE REJESTRACJA = ?";
            $q = $pdo->prepare($sql);
            $q->execute(array($REJESTRACJA,$CZY_USZKODZONY,strtoupper($MARKA),$REJESTRACJA));
            Database::disconnect();
            header("Location: samochodyindex.php");
        }
    } else {
        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM SAMOCHODY where REJESTRACJA = ?";
        $q = $pdo->prepare($sql);
...
<body>
    <div class="container">
                ...
                <div class="span10 offset1">
                    ...
                    <form class="form-horizontal" action="samochodyupdate.php?REJESTRACJA=<?php echo $REJESTRACJA?>" method="post">
                      ...
                      <div class="form-actions">
                          ...
                          <a class="btn" href="samochodyindex.php">Back</a>
                        </div>
                    </form>
                </div>
